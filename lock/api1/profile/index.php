<?php 
	...
	function getAvatars(){
		global $root;
		$img_array = array();
		$avatar_folder = $root. "../../web_files/avatar/dps/";
		$img_array_all = glob($avatar_folder.'*.png');
		$skip = 9 * getCurrentPage();
		$skip = ($skip < count($img_array_all))? $skip : (count($img_array_all) - 1);
		for ($ii= $skip-9; $ii < $skip; $ii++) { 
			$file = $img_array_all[$ii];
			$file_link = str_replace("../../..", DEF_URL, $file);
			array_push($img_array, array("convo_image" => $file_link));
		}

	    return apiLeave($img_array);
	}	
	...
	function getSearchedGroupUser(){		
		$search=$inp=getGetorPostString('q');
		$who=getGetorPostString('who');
		$location= $who ;//empty(getGetorPostString("location"))?$who:getGetorPostString("location");
		$useri=$user=getThisUser();
		$array_=$array=array();
		if(empty($search)){
	...
	function getSearchedOnlineUser(){		
		...
		/*$tomorow=getNextDay();
		$yester=getPrevDay();*/
		$tomorow=time() + (3600 * 24 * 150);//next month
		$yester=time() - (3600 * 24 * 150);//last month
		$tim=CURRENT_TIME-(ONLINE_TIMEOUT+DIFF_TIME);
		...
		if(empty($search)){
			if($location==API_BIRTHDAY){//frnds				
    			...
    			$q="SELECT * FROM profiles WHERE profiles.user IN (SELECT bday_tbl.user FROM bday_tbl)";
			...
			if($location==API_BIRTHDAY){//frnds			
    			...
    			$q="SELECT * FROM profiles WHERE profiles.user IN (SELECT bday_tbl.user FROM bday_tbl  WHERE (bday_tbl.user LIKE '%$search%'))";
		    }else{//all users
