<?php	
	...
	function getRequest(){
		return getGetorPostString(("request"));
	}

	function verifyAppid(){
		$id=getGetorPostString("acc_id");
		if ($id==APP_ID_){
			return true;
		}
		return false;		
	...
	function release($var){
		...
		$json_res=json_encode($var);
		print_r($json_res);
		if(SAVE_API_RESULT){
		    fwrite(fopen(DUMP_DIR."api_result.txt", "a+"), $json_res);
		}
		exit();
	}
	
	function saveThis($var){
	    if(is_array($var)){
	        $json_res=json_encode($var);
	    }else{
	        $json_res=($var);
	    }
		fwrite(fopen(DUMP_DIR."api_store.txt", "a+"), " ".$json_res." ");
	}
